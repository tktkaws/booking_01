      ...
      <form method="dialog" className="flex max-h-[80vh] flex-col" onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="space-y-3 overflow-y-auto px-6 py-6 text-sm">
          <label className="flex flex-col gap-1">
            <span className="text-sm font-semibold text-slate-800">表示名</span>
            <input
              type="text"
              value={displayName}
              onChange={(e) => setDisplayName(e.target.value)}
              className="w-full rounded-md border border-slate-300 px-3 h-9 grid items-center focus:border-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-200"
              placeholder="お名前を入力"
            />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-sm font-semibold text-slate-800">部署</span>
            <select
              value={departmentId}
              onChange={(e) => setDepartmentId(e.target.value)}
              className="w-full rounded-md border border-slate-300 bg-white px-3 h-9 grid items-center focus:border-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-200"
            >
              {departments.map((d) => (
          ...
          <div className="space-y-3">
            <div className="text-sm font-semibold text-slate-800">カラー設定</div>
                  ...
                  <div key={d.id} className="rounded-lg border border-slate-200 p-3">
                    <div className="grid grid-cols-[1fr_auto_auto_auto] gap-3 items-center">
                      ...
                      <input
                        ...
                        className="w-36 rounded-md border border-slate-300 px-3 h-9 grid items-center text-sm"
                      />
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="rounded px-3 h-9 grid items-center hover:ring-2"
                      >
                        <RotateCcw />
                      </button>
                    </div>
